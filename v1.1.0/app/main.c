/*
 * mlb_scores - MLB live score ACAP v1.1.0
 * Vendor: gscarlet22 design for Axis C1720 / C1710
 *
 * Multi-team: up to 8 teams monitored simultaneously.
 * Per-team audio clips. Display fires only on score or inning change.
 * Migrates automatically from 1.0.x single-team AXParameter layout.
 *
 * API used:
...
static void poll_one_team(int idx) {
    ...
    if (strcmp(live.state, "Final") == 0 && !g_app.display_persist_final &&
        ...
        /* No persist: clear immediately on next pass (final_detected_at stays 0,
           so poll_one_team will call reset next time final is seen and persist is off).
           Set final_detected_at briefly to trigger reset at top of next call. */
        state->final_detected_at = time(NULL);
    }
}
...
static void handle_request(int fd) {
    ...
    if (strcmp(method, "GET") == 0 && ROUTE("/status")) {
        ...
        for (int i = 0; i < g_app.num_teams; i++) {
            ...
            char safe_lp[MAX_MSG * 2];
            json_escape(s->last_play, safe_lp, sizeof(safe_lp));
            ...
            cJSON_AddStringToObject(team, "last_play",         safe_lp);
    ...
    if (strcmp(method, "GET") == 0 && ROUTE("/config")) {
        ...
        cJSON_AddStringToObject(root, "poll_interval_sec",    "");
        {
            char tmp[16]; snprintf(tmp, sizeof(tmp), "%d", g_app.poll_interval_sec);
            cJSON_ReplaceItemInObject(root, "poll_interval_sec", cJSON_CreateString(tmp));
        }
        cJSON_AddStringToObject(root, "display_enabled",      g_app.display_enabled ? "true" : "false");
        cJSON_AddStringToObject(root, "display_text_size",    g_app.display_text_size);
        cJSON_AddStringToObject(root, "display_text_color",   g_app.display_text_color);
        cJSON_AddStringToObject(root, "display_bg_color",     g_app.display_bg_color);
        {
            char tmp[16]; snprintf(tmp, sizeof(tmp), "%d", g_app.display_scroll_speed);
            cJSON_AddStringToObject(root, "display_scroll_speed", tmp);
        }
        cJSON_AddStringToObject(root, "display_persist_final",g_app.display_persist_final ? "true" : "false");
        {
            char tmp[16]; snprintf(tmp, sizeof(tmp), "%d", g_app.display_duration_ms);
            cJSON_AddStringToObject(root, "display_duration_ms", tmp);
        }
...
#define AXSET(name, val) ax_parameter_set(p, name, val, TRUE, NULL)
    ...
    if (strcmp(method, "POST") == 0 && ROUTE("/test_display")) {
        const char *tname = (g_app.num_teams > 0) ? g_app.state[0].team_name : "MLB";
        char tmsg[MAX_MSG];
        snprintf(tmsg, sizeof(tmsg), "MLB SCORES TEST: %s Score Display", tname);
        char disp_resp[512] = "";
    ...
    if (strcmp(method, "POST") == 0 && ROUTE("/test_audio")) {
        int clip_id = (g_app.num_teams > 0) ? g_app.teams[0].notify_clip_id : 38;
        char clip_resp[512] = "";
